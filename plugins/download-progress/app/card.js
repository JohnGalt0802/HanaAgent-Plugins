...
(function () {
  ...
  function render(t) {
    ...
    html += '<span class="dl-name" title="' + esc(t.fileName || "") + '">' + esc(t.fileName || "") + "</span>";
    ...
    if (pending || running) {
      ...
      if (t.kind === "command") {
        ...
        html += '<button class="dl-btn dl-copy" id="dl-copy">复制路径</button>';
      } else {
        html += '<button class="dl-btn primary" id="dl-open">打开</button>';
        html += '<button class="dl-btn" id="dl-folder" title="打开所在文件夹">文件夹</button>';
        html += '<button class="dl-btn dl-copy" id="dl-copy">复制路径</button>';
      }
    }
    html += "</div>";
    ...
    html += '<span class="dl-size">' + esc(sizeText2) + "</span>";
    html += "</div>";
